# アリ進化ゲーム v0.10 プロトタイプ

実在するアリの適応を進化列へ積み、目先の繁栄、常に起こる問題への防御、将来の環境変化への最適化を競わせる5ラウンドの実験ゲームです。

現在の版では、8枚からseedで選んだ5枚の環境を最初から公開します。各ラウンドの開始時には「襲撃」と「衛生」を振り、カードの起動、防御、タグ構築を考えて対応します。通常は各1d4ですが、最適化を要求しない一部の環境では振り方や補正が変化します。問題減点は今ラウンドの繁栄から先に引き、残りへ小型1～超大型4の倍率を掛けます。

...
function effectHTML(effect) {
  ...
  if (effect.prosperity) parts.push(`基礎繁栄 +${effect.prosperity}`);
  ...
  if (effect.store_hand_card) parts.push(`手札1枚を貯蔵（次ラウンド以降、毎ラウンド基礎繁栄 +${effect.storage_income_per_card}）`);
  (effect.tag_prosperity || []).forEach((bonus) => parts.push(`盤面の${esc(bonus.name || bonus.tag)}1つごとに基礎繁栄 +${bonus.coefficient}（合計上限${effect.tag_prosperity_cap}）`));
...
function renderOverview() { const environment = state.environment; $("#overview").innerHTML = `${resultHTML(state.last_result)}<h2>ラウンド ${state.round}/5・${esc(environment.name)}</h2><p>${esc(environment.text)}</p><div class="stats"><div class="stat">繁栄<strong>${state.prosperity}</strong></div><div class="stat">サイズ<strong>${esc(state.size_name)}</strong></div><div class="stat">今ラウンドの基礎<strong>${state.round_prosperity_base}</strong></div><div class="stat">サイズ倍率<strong>×${state.round_prosperity_multiplier}</strong></div><div class="stat">今ラウンド獲得予定<strong>+${state.round_prosperity_delta}</strong></div><div class="stat">手札<strong>${state.hand.length}/${state.hand_limit}</strong></div><div class="stat">次ラウンド保持<strong>+${state.pending_retention_bonus || 0}</strong></div></div><h3>毎ラウンド独立の2問題（各1d4）</h3><div class="problem-grid">${problemsHTML(state.problems)}</div>${optimizationHTML(state.optimizations, true)}${problemRulesHTML(environment.problem_roll_rules)}`; }

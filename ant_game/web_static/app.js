...
function handCardHTML(card) {
  const actions = state.columns.map((column) => { const option = (card.placement_options || [])[column.index]; const status = option?.status || "other"; const title = status === "ready" ? "配置後すぐ強効果を起動できます" : status === "one-short" ? "配置後、強効果まであと1タグです" : "配置後も強効果の条件が不足します"; return `<button class="play-placement placement-${status}" title="${title}" onclick="act('play',{card_id:'${esc(card.id)}',column:${column.index}})">列${column.index + 1}へ配置</button><button class="support-placement" onclick="act('support',{card_id:'${esc(card.id)}',column:${column.index}})">列${column.index + 1}の支援</button>`; }).join("");

...
function effectHTML(effect) {
  ...
  (effect.tag_prosperity || []).forEach((bonus) => { const total = `盤面の${esc(bonus.name || bonus.tag)}1つごとに繁栄 +${bonus.coefficient}`; const divisor = effect.tag_prosperity_divisor > 1 ? `、合計を${effect.tag_prosperity_divisor}で割って切り捨て` : ""; const cap = effect.tag_prosperity_cap != null ? `、上限${effect.tag_prosperity_cap}` : "（上限なし）"; parts.push(total + divisor + cap); });
  (effect.shields || []).forEach((shield) => parts.push(`${esc(shield.name || problemNames[shield.problem_id] || shield.problem_id)}シールド +${shield.amount}`));
  (effect.vulnerabilities || []).forEach((item) => parts.push(`${esc(item.name || problemNames[item.problem_id] || item.problem_id)}脆弱性 +${item.amount}`));
  ...
  (effect.size_effects || []).forEach((sizeEffect) => {
    ...
    (sizeEffect.shields || []).forEach((shield) => details.push(`${esc(shield.name || problemNames[shield.problem_id] || shield.problem_id)}シールド +${shield.amount}`));
    (sizeEffect.vulnerabilities || []).forEach((item) => details.push(`${esc(item.name || problemNames[item.problem_id] || item.problem_id)}脆弱性 +${item.amount}`));
...
function cardHTML(card, selectable = false) {
  ...
  const pushed = card.on_pushed_out ? `<div class="fallback"><span class="label">押し出し時効果</span>${effectHTML(card.on_pushed_out)}</div>` : "";
  return `<article class="card" style="--tag-band:${cardBand(card.tags)}" ${selectable ? `data-keep="${esc(card.id)}" tabindex="0" role="button"` : ""}><div class="meta">${esc(card.role)}</div><h3>${esc(card.name)}</h3><p>${esc(card.text)}</p>${biology}<div class="self-tags"><span class="label">カードのタグ</span><div class="tag-row">${card.tags.map((tag) => tagHTML(tag)).join("") || "なし"}</div></div><div class="required-tags condition"><span class="label">起動条件</span>${requirementsHTML(card.requirements)}</div><div class="effect-row">${card.options.map(effectHTML).join("") || "<span class='effect'>起動効果なし</span>"}</div>${fallback}${pushed}</article>`;
}
...
function problemsHTML(problems) {
  return problems.map((problem) => { const raw = problem.raw_rolls?.length ? problem.raw_rolls.join(", ") : "－"; const rule = problem.roll_source === "previous_round" ? `前ラウンド値 ${problem.selected_roll}` : problem.roll_source === "d4_first_round" ? `初回d4 ${raw}` : problem.raw_rolls?.length > 1 ? `出目 ${raw} → ${problem.combine === "sum" ? "合計" : "高い方"} ${problem.selected_roll}` : `出目 ${raw}`; const modifier = problem.modifier ? ` +${problem.modifier}` : ""; return `<article class="problem"><strong>${esc(problem.name)}</strong><span>${rule}${modifier} = <b>${problem.effective_roll ?? problem.roll ?? "?"}</b></span><span>シールド：${problem.defense ?? problem.shield ?? 0}</span><span>未防御：${problem.unblocked ?? "?"}</span>${problem.penalty != null ? `<span class="${problem.penalty ? "warning" : "success"}">問題ペナルティ：${problem.penalty}</span>` : ""}</article>`; }).join("");
}
...
function problemRulesHTML(rules) {
  ...
  return `<p class="problem-rule">独立問題の強化：${entries.map(([id, rule]) => `${names[id] || id} ${rule.previous_round_bonus != null ? `前ラウンド値 +${rule.previous_round_bonus}（上限なし）` : `${rule.rolls > 1 ? `${rule.rolls}d4（${rule.combine === "sum" ? "合計" : "高い方"}）` : "1d4"}${rule.bonus ? ` +${rule.bonus}` : ""}`}`).join(" ／ ")}</p>`;
}
...
function renderForecast() { $("#forecast").innerHTML = `<h2>公開された5ラウンドの環境変化予報</h2><div class="forecast-grid">${state.forecast.map((environment) => `<article class="forecast-card ${environment.current ? "current" : ""} ${environment.completed ? "completed" : ""}"><div class="meta">ラウンド ${environment.round}${environment.finale ? "・最終環境" : ""}${environment.current ? "・現在" : ""}</div><h3>${esc(environment.name)}</h3><p>${esc(environment.text)}</p>${optimizationHTML(environment.optimizations)}${problemRulesHTML(environment.problem_roll_rules)}</article>`).join("")}</div>`; }
function renderOverview() { const environment = state.environment; $("#overview").innerHTML = `${resultHTML(state.last_result)}<h2>ラウンド ${state.round}/5・${esc(environment.name)}</h2><p>${esc(environment.text)}</p><div class="stats"><div class="stat">繁栄<strong>${state.prosperity}</strong></div><div class="stat">サイズ<strong>${esc(state.size_name)}</strong></div><div class="stat">今ラウンドの繁栄<strong>${state.round_prosperity_base}</strong></div><div class="stat">サイズ倍率<strong>×${state.round_prosperity_multiplier}</strong></div><div class="stat">今ラウンド獲得予定<strong>+${state.round_prosperity_delta}</strong></div><div class="stat">手札<strong>${state.hand.length}/${state.hand_limit}</strong></div><div class="stat">次ラウンド保持<strong>+${state.pending_retention_bonus || 0}</strong></div><div class="stat">次ラウンド公開追加<strong>+${state.pending_candidate_bonus || 0}枚</strong></div></div><h3>毎ラウンド独立の2問題</h3><div class="problem-grid">${problemsHTML(state.problems)}</div>${optimizationHTML(state.optimizations, true)}${problemRulesHTML(environment.problem_roll_rules)}`; }
...
function renderDecision() {
  ...
  if (state.phase === "retain") { const hand = state.hand.length ? `<div class="cards">${state.hand.map((card) => cardHTML(card)).join("")}</div>` : "<p>手札はありません。</p>"; const expansion = state.retention_trade_used ? "<span class='meta'>候補追加を使用済み</span>" : `<button id="expand-candidates" ${state.can_expand_candidates ? "" : "disabled"}>保持上限−1で候補を2枚追加</button>`; box.innerHTML = `<h2>2. 公開カードを保持 <span id="keep-count">0/${state.retention_limit}</span></h2><p class="phase-help">カードを選び、保持上限まで手札に加えます。今回の公開は${state.candidate_draw_count || state.candidates.length}枚です。候補追加は各ラウンド1回だけ使えます。</p><div class="button-row">${expansion}</div><h3>今回の候補</h3><div class="cards">${state.candidates.map((card) => cardHTML(card, true)).join("")}</div><h3 class="hand-heading">現在の手札</h3>${hand}<div class="button-row"><button class="primary" id="keep-button">保持して進む</button></div>`; bindCandidateSelection(); if ($("#expand-candidates")) $("#expand-candidates").addEventListener("click", () => act("expand_candidates")); return; }
...
function columnHTML(column) { const cards = column.cards.map((card) => { const pushed = card.on_pushed_out ? `<div><span class="label">押し出し時効果</span>${effectHTML(card.on_pushed_out)}</div>` : ""; return `<div class="stack-card ${card.top ? "top" : ""} ${card.support ? "support" : ""}"><strong>${esc(card.name)}</strong><br><small>${card.top ? "最上段" : card.support ? "支援" : "下段"}${card.activated ? "・起動済み" : ""}${card.stored_count ? `・伏せ貯蔵 ${card.stored_count}枚（次回以降、毎ラウンド繁栄 +${card.storage_income_next_round}）` : ""}</small><div class="tag-row">${card.tags.map((tag) => tagHTML(tag)).join("")}</div>${pushed}</div>`; }).join(""); const activations = column.activations.map((activation) => { const click = activation.requires_storage_target || activation.requires_recovery_target ? "" : `onclick="act('activate',{column:${column.index},option:${activation.option}})"`; const recovery = activation.requires_recovery_target && activation.enabled ? activation.target_candidates.map((target) => `<button class="recovery-target" onclick="act('activate',{column:${column.index},option:${activation.option},target_card_id:'${esc(target.id)}'})">${esc(target.name)}を回収して起動</button>`).join("") : ""; return `<div><button ${activation.enabled && !activation.requires_storage_target && !activation.requires_recovery_target ? "" : "disabled"} ${click} title="${esc(activation.reason)}">${activation.tier === "fallback" ? "弱効果：" : activation.requires_storage_target ? "手札から貯蔵：" : activation.requires_recovery_target ? "下段を回収：" : "起動："}${effectHTML(activation.effect)}</button>${activation.requirements.length ? `<div class="condition">条件：${requirementsHTML(activation.requirements, true)}</div>` : ""}${activation.requires_storage_target ? "<small>手札のカードから「貯蔵して起動」を選択</small>" : ""}${recovery}</div>`; }).join(""); return `<article class="column"><h3>${esc(column.name)} <small>${column.cards.length}/${column.capacity}</small></h3><span class="label">列の全タグ（最適化）</span><div class="tag-row">${column.tags.map((tag) => tagHTML(tag)).join("") || "なし"}</div><span class="label">起動に使うタグ</span><div class="tag-row">${column.activation_tags.map((tag) => tagHTML(tag)).join("") || "なし"}</div><div class="stack">${cards || "空"}</div>${column.next_pushed ? `<p class="warning">次に押し出されるカード：${esc(column.next_pushed)}</p>` : ""}<div class="button-row">${activations}</div></article>`; }

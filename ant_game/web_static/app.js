...
function problemRulesHTML(rules) {
  ...
  return `<p class="problem-rule">独立問題の強化：${entries.map(([id, rule]) => `${names[id] || id} ${rule.previous_round_bonus != null ? `前ラウンド値 +${rule.previous_round_bonus}（上限なし）` : `${rule.rolls > 1 ? `${rule.rolls}d4（高い方）` : "1d4"}${rule.bonus ? ` +${rule.bonus}` : ""}`}`).join(" ／ ")}</p>`;
}
...
function renderOverview() { const environment = state.environment; $("#overview").innerHTML = `${resultHTML(state.last_result)}<h2>ラウンド ${state.round}/5・${esc(environment.name)}</h2><p>${esc(environment.text)}</p><div class="stats"><div class="stat">繁栄<strong>${state.prosperity}</strong></div><div class="stat">サイズ<strong>${esc(state.size_name)}</strong></div><div class="stat">今ラウンドの繁栄<strong>${state.round_prosperity_base}</strong></div><div class="stat">サイズ倍率<strong>×${state.round_prosperity_multiplier}</strong></div><div class="stat">今ラウンド獲得予定<strong>+${state.round_prosperity_delta}</strong></div><div class="stat">手札<strong>${state.hand.length}/${state.hand_limit}</strong></div><div class="stat">次ラウンド保持<strong>+${state.pending_retention_bonus || 0}</strong></div><div class="stat">次ラウンド公開追加<strong>+${state.pending_candidate_bonus || 0}枚</strong></div></div><h3>毎ラウンド独立の2問題（各1d4）</h3><div class="problem-grid">${problemsHTML(state.problems)}</div>${optimizationHTML(state.optimizations, true)}${problemRulesHTML(environment.problem_roll_rules)}`; }

...
class WebGameService:
    ...
    def _option_data(option: ActionOption) -> dict[str, Any]:
        return {
            ...
            "size_effects": [{"size": item.size.name.lower(), "prosperity": item.prosperity, "environment_prosperity_loss_reduction": item.environment_prosperity_loss_reduction, "shields": [{"problem_id": shield.problem_id, "amount": shield.amount} for shield in item.shields], "vulnerabilities": [{"problem_id": vulnerability.problem_id, "amount": vulnerability.amount} for vulnerability in item.vulnerabilities]} for item in option.size_effects],
        }

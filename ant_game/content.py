"""Data-driven ant evolution cards and forecastable disasters.
 ...
 six evolutionary root tags; recurring problems are limited to raid, fungal,
...
ROOT_TAGS = frozenset({"Morphology", "Chemistry", "Sociality", "Nesting", "Movement", "Resource Ecology"})
...
NORMAL_TRAITS: tuple[TraitCard, ...] = (
    ...
    _action("oecophylla_living_chain", "Oecophylla Living Chain", frozenset({"Sociality", "Movement"}), {"Sociality": 1, "Movement": 1}, (ActionOption(draw_cards=1, text="Form a chain and extend the route."),), "Oecophylla smaragdina", "Workers form pulling and bridging chains to move across gaps and bend leaves.", "https://pmc.ncbi.nlm.nih.gov/articles/PMC3997362/", "Payoff", "A chain converts many bodies into a temporary bridge."),
    ...
    _action("odontomachus_tension_lock", "Odontomachus Tension Lock", frozenset({"Morphology", "Movement"}), {}, (ActionOption(prosperity=1, text="Snap at prey or rivals."), ActionOption(shields=(_shield("raid", amount=1),), text="Use the snap to launch away.")), "Odontomachus bauri and O. brunneus", "A latch-spring mandible strikes prey and can propel an ant away from danger.", "https://pmc.ncbi.nlm.nih.gov/articles/PMC1568925/", "Bridge", "One mechanical innovation supports both attack and escape."),
    ...
    _action("megaponera_field_medicine", "Megaponera Field Medicine", frozenset({"Chemistry", "Sociality"}), {"Movement": 3, "Chemistry": 1}, (ActionOption(shields=(_shield("fungal", amount=3),), text="Treat an infected wound."),), "Megaponera analis", "Workers apply antimicrobial gland secretions to infected wounds and sharply reduce mortality.", "https://pmc.ncbi.nlm.nih.gov/articles/PMC10756881/", "Payoff", "Chemical care turns group hunting into recoverable risk."),
    _action("megaponera_rescue_column", "Megaponera Rescue Column", frozenset({"Sociality", "Movement"}), {}, (ActionOption(shields=(_shield("raid", amount=1),), text="Carry an injured nestmate home."),), "Megaponera analis", "Returning raiders carry injured nestmates back to the nest after termite fights.", "https://pmc.ncbi.nlm.nih.gov/articles/PMC5389746/", "Bridge", "A moving group preserves the value of experienced workers."),
    ...
    _action("cataglyphis_sky_compass", "Cataglyphis Sky Compass", frozenset({"Movement", "Resource Ecology"}), {"Movement": 2, "Resource Ecology": 1}, (ActionOption(draw_cards=1, text="Discover another option by the shortest known route."),), "Cataglyphis fortis", "Desert foragers use path integration and polarized skylight to return across featureless terrain.", "https://pmc.ncbi.nlm.nih.gov/articles/PMC1567920/", "Payoff", "Navigation preserves a resource route when landmarks disappear."),
    ...
    _action("solenopsis_raft_cycling", "Solenopsis Raft Cycling", frozenset({"Sociality", "Movement"}), {}, (ActionOption(shields=(_shield("nest_damage", amount=1),), text="Cycle workers through a living raft."),), "Solenopsis invicta", "Workers cycle through floating rafts and protect brood during prolonged flooding.", "https://pmc.ncbi.nlm.nih.gov/articles/PMC3462402/", "Bridge", "Collective buoyancy makes local flooding survivable without permanent safety."),
    ...
    _action("temnothorax_emergency_emigration", "Temnothorax Emergency Emigration", frozenset({"Movement", "Nesting"}), {}, (ActionOption(shields=(_shield("nest_damage", amount=1),), text="Move before the cavity becomes unsafe."),), "Temnothorax albipennis", "Colonies emigrate from fragile cavities when their current nest is damaged or deteriorates.", "https://pmc.ncbi.nlm.nih.gov/articles/PMC3101226/", "Bridge", "Movement and nesting together allow a timely escape."),
)


DISASTERS: tuple[DisasterCard, ...] = (
    DisasterCard("flood", "Flood", OptimizationRequirement("Flood Adaptation", {"Sociality": 3, "Morphology": 1, "Movement": 3, "Nesting": 1, "Resource Ecology": 1}, "Solenopsis invicta; Cephalotes atratus and arboreal ants", "Fire ants interlock into buoyant rafts, while arboreal ants use gliding and canopy routes above inundated ground.", "https://pmc.ncbi.nlm.nih.gov/articles/PMC3462402/", "A single flood adaptation combines a living raft with a complete canopy retreat."), "Floodwater tears through ground routes and separates canopy refuges."),
    ...
    DisasterCard("habitat_instability", "Habitat Instability", OptimizationRequirement("Emergency Emigration", {"Sociality": 2, "Movement": 2, "Nesting": 1}, "Temnothorax albipennis", "Colonies use collective nest-site decisions and emigrate when fragile cavities are damaged or deteriorate.", "https://pmc.ncbi.nlm.nih.gov/articles/PMC3101226/", "A colony that can move together before its cavity fails retains a viable home."), "Nest cavities become unsafe as substrate and shelter deteriorate."),
    DisasterCard("landmark_loss", "Landmark Loss", OptimizationRequirement("Sky Compass Navigation", {"Movement": 3, "Resource Ecology": 1}, "Cataglyphis fortis", "Desert foragers use path integration and polarized skylight to return across featureless terrain.", "https://pmc.ncbi.nlm.nih.gov/articles/PMC1567920/", "A celestial route preserves foraging when familiar landmarks disappear."), "Wind and shifting substrate erase the routes used to reach food."),
)

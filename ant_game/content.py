...
NORMAL_TRAITS: tuple[TraitCard, ...] = (
    ...
    _action("megaponera_field_medicine", "Megaponera Field Medicine", frozenset({"Chemistry", "Sociality"}), {"Sociality": 3, "Chemistry": 1}, (ActionOption(shields=(_shield("sanitation", amount=3),), text="Treat an infected wound."),), "Megaponera analis", "Workers apply antimicrobial gland secretions to infected wounds and sharply reduce mortality.", "https://pmc.ncbi.nlm.nih.gov/articles/PMC10756881/", "Payoff", "Chemical treatment becomes reliable only when a coordinated care group can deliver it.", (ActionOption(shields=(_shield("sanitation", amount=1),), text="Apply a limited wound treatment."),)),
    ...
    _action("pheidole_raid_wall", "Pheidole Raid Wall", frozenset({"Sociality", "Morphology"}), {"Nesting": 3, "Morphology": 1}, (ActionOption(shields=(_shield("raid", amount=3),), text="Coordinate supermajors at the entrance."),), "Pheidole obtusospinosa", "Supermajors and workers use multiple defensive phases against invading army ants.", "https://academic.oup.com/jinsectscience/article/10/1/1/820704", "Payoff", "Sociality and collective defense combine against a known rival route.", (ActionOption(shields=(_shield("raid", amount=1),), text="Coordinate a smaller defensive line."),)),
    ...
    _action("ooceraea_synchronized_brood", "Ooceraea Synchronized Brood", frozenset({"Chemistry", "Sociality"}), {"Resource Ecology": 2}, (ActionOption(draw_cards=1, text="Synchronize reproduction and reveal another route."),), "Ooceraea biroi", "Clonal raider ant colonies alternate synchronized reproductive and brood-care or foraging phases.", "https://pmc.ncbi.nlm.nih.gov/articles/PMC8244912/", "Payoff", "A colony-wide reproductive rhythm makes the next foraging window predictable."),
    _action("mycocepurus_clonal_garden", "Mycocepurus Clonal Garden", frozenset({"Sociality", "Resource Ecology"}), {"Nesting": 2}, (ActionOption(prosperity=2, text="Harvest a garden maintained by clonal workers."),), "Mycocepurus smithii", "Documented asexual Mycocepurus smithii populations combine thelytokous queen reproduction with obligate fungus farming.", "https://pmc.ncbi.nlm.nih.gov/articles/PMC2686657/", "Payoff", "Clonal continuity and fungus cultivation reinforce a specialized food base."),
    _action("cardiocondyla_dual_males", "Cardiocondyla Dual Males", frozenset({"Morphology", "Sociality"}), {"Resource Ecology": 2}, (ActionOption(prosperity=2, text="Use winged and wingless males for two reproductive routes."), ActionOption(draw_cards=1, text="Open a reproductive alternative.")), "Cardiocondyla obscurior", "Cardiocondyla obscurior produces wingless fighter males that remain in the nest and peaceful winged males that can disperse.", "https://pmc.ncbi.nlm.nih.gov/articles/PMC3066177/", "Payoff", "Two male morphs preserve both local competition and dispersal."),
...
DISASTERS: tuple[DisasterCard, ...] = (
    EnvironmentCard(
        "flood", "Flood", (
            OptimizationRequirement("Living Raft", {"Sociality": 4, "Morphology": 2}, "Solenopsis invicta", "Fire ants interlock into buoyant rafts and cycle workers through the water.", "https://pmc.ncbi.nlm.nih.gov/articles/PMC3462402/", "Collective buoyancy keeps brood above floodwater."),
            OptimizationRequirement("Canopy Refuge", {"Nesting": 4, "Morphology": 2, "Resource Ecology": 2}, "Cephalotes atratus and arboreal ants", "Arboreal ants steer falls and use canopy routes above inundated ground.", "https://pmc.ncbi.nlm.nih.gov/articles/PMC2880152/", "A high refuge avoids the flooded ground entirely."),
        ), {}, "Floodwater tears through ground routes and separates canopy refuges."),
    EnvironmentCard(
        "desert_heat_wave", "Desert Heat Wave", (
            OptimizationRequirement("Silver Hair", {"Morphology": 5}, "Cataglyphis bombycina", "Triangular reflective hairs reduce solar heating under intense sunlight.", "https://pmc.ncbi.nlm.nih.gov/articles/PMC4830450/", "Reflective body structure sustains a hot run."),
            OptimizationRequirement("Heat Shock", {"Chemistry": 5}, "Cataglyphis bombycina", "Constitutive and inducible heat-shock responses protect cellular machinery.", "https://pmc.ncbi.nlm.nih.gov/articles/PMC6003908/", "Cellular protection supports activity at extreme temperature."),
        ), {}, "Radiant heat and dry ground punish exposed foragers."),
    EnvironmentCard(
        "prolonged_drought", "Prolonged Drought", (
            OptimizationRequirement("Underground Granary", {"Nesting": 4, "Resource Ecology": 3}, "Pogonomyrmex badius", "Workers store and process seeds in protected subterranean chambers.", "https://pmc.ncbi.nlm.nih.gov/articles/PMC5125654/", "A deep granary separates stored food from the dry surface."),
            OptimizationRequirement("Stored Bodies", {"Sociality": 4, "Resource Ecology": 3}, "Myrmecocystus spp.", "Replete workers store liquid food in their crops for long periods.", "https://pmc.ncbi.nlm.nih.gov/articles/PMC3127378/", "Living storage buffers a colony through a dry spell."),
        ), {}, "A long dry spell removes easy surface food."),
    EnvironmentCard(
        "habitat_instability", "Habitat Instability", (
            OptimizationRequirement("Quorum Emigration", {"Sociality": 4, "Nesting": 3}, "Temnothorax albipennis", "Scouts use quorum-like feedback to choose a new cavity before the old one fails.", "https://pmc.ncbi.nlm.nih.gov/articles/PMC2817311/", "Collective site choice makes emigration timely."),
            OptimizationRequirement("Canopy Rebuild", {"Nesting": 4, "Sociality": 2, "Morphology": 2}, "Oecophylla smaragdina", "Workers pull leaves together with larval silk to rebuild a living canopy nest.", "https://pmc.ncbi.nlm.nih.gov/articles/PMC4896424/", "A rebuilt canopy replaces a failing ground shelter."),
        ), {}, "Substrate and shelter deteriorate while colonies compete for safe cavities."),
    EnvironmentCard(
        "landmark_loss", "Landmark Loss", (
            OptimizationRequirement("Sky Compass", {"Resource Ecology": 5, "Morphology": 1}, "Cataglyphis fortis", "Desert foragers use path integration and polarized skylight to return across featureless terrain.", "https://pmc.ncbi.nlm.nih.gov/articles/PMC1567920/", "A celestial route preserves access when familiar landmarks disappear."),
            OptimizationRequirement("Pheromone Web", {"Chemistry": 4, "Sociality": 3}, "Formicidae", "Ants reinforce chemical trails and recruit nestmates to preserve routes.", "https://pmc.ncbi.nlm.nih.gov/articles/PMC3772619/", "A redundant chemical network replaces erased visual landmarks."),
